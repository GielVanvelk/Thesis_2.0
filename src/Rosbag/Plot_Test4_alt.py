...
timeF_kz = [x - start_time for x in timeF_kz]
timeP_kz = [x - start_time for x in timeP_kz]
...
fitted1_alt = []
fitted2_alt = []
for i in range(len(fitted_2)-1):
	if fitted_2[i] >= -4:
		fitted2_alt.append(fitted_2[i])
		fitted1_alt.append(fitted_1[i])
...
if create_figures == True:
	...
	lns1 = ax.scatter(timeP_kz, poses_kz_mm, c='b', label = 'Z-coordinate')
	ax2 = ax.twinx()
	lns3 = ax2.plot(timeF_kz, forces_kz, 'r', label = 'Z-force')
	...
	plt.title('Z-Force and Z-Position data during stiffness calculation(%s)' % FILENAME)
	...
	fit = np.polyfit(fitted1_alt, fitted2_alt, 1)
	fit_slope = fit[0]*1000
	y_new = np.polyval(fit, fitted1_alt)

	fig = plt.figure()
	ax = fig.add_subplot(111)
	lns1 = ax.scatter(fitted1_alt, fitted2_alt, c='r', label = 'Fz(z-coordinate)')
	#lns1 = ax.plot(fitted_1, fitted_2, 'r', label = 'Fz(z-coordinate)')
	lns2 = ax.plot(fitted1_alt, y_new, '-', label = 'Polyfitted Line with slope %i' % fit_slope)
