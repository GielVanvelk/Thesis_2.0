...
starting_pose = {-0.4, -0.07, 0.25, 3.14, 0, 3.14}  --[m]
end_pose = {-0.35, 0.07, 0.25, 3.14, 0, 3.14} 		--[m]
...
a_k_calc_force_start = 0.5
custom_stiffness = 100
...
return rfsm.state {
	...
	error = rfsm.state {
	   entry=function()
		   ...
		   stiffness = gcomp_gui:getProperty("stiffness")
		   m = stiffness:get()
		   print(m)
	   end,
	   exit=function()
	...
	apply_force = rfsm.state {
		entry=function()
			...
			stiffness_calc_force_start:set(a_k_calc_force_start)
			...
			etaslcore:set_etaslvar("maxvel", 0.02)
			etaslcore:set_etaslvar("maxacc", 0.02)
			etaslcore:set_etaslvar("eq_r",0.08)
			etaslcore:set_etaslvar("delta_x", 0)
			etaslcore:set_etaslvar("delta_y", 0)
			etaslcore:set_etaslvar("delta_z", 0.2)
		...
		exit=function()
			...
			if m == 0 then
				stiffness:set(custom_stiffness)
			end
			print(m)
	...
	scanning = rfsm.state {
	  entry=function(fsm)
		  ...
		  stiffness = gcomp_gui:getProperty("stiffness")
		  stiffness_val = stiffness:get()

		  rtt.sleep(2,0) --Sleep for 2 seconds
		  state_scanning(end_pose, scanning_vel, max_acc, force_setpoint, stiffness_val)
		  rtt.sleep(2,0) --Sleep for 2 seconds
	  end,
	  exit=function()
		etaslcore:stop()
	...
	rfsm.trans {src="apply_force",               tgt="scanning",   				   events={'e_finished@etaslcore'}  },
	rfsm.trans {src="apply_force",               tgt="scanning",   				   events={'e_forceReached'}        },
