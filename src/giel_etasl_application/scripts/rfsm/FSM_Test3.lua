   ...
   get_robot_ready = rfsm.state {
	...
	checking_control_parameters = rfsm.state {
		entry=function(fsm)
			...
			stiffness_auto = k:get()

			print('The stiffness that will be used is:')
			print(stiffness_auto)
			if stiffness_custom > 800 then
	...
	scanning = rfsm.state {
		entry=function()
			...
			stiffness_val = k:get()
			K_val = K_controller
			state_scanning(end_pose, scanning_vel, max_acc, force_setpoint, stiffness_auto, force_torque_limits, controller_gain)
		end,
		exit=function()
			etaslcore:stop()
			etaslcore:cleanup()
