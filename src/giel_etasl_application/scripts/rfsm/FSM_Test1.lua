   ...
   get_robot_ready = rfsm.state {
	...
	stiffness_test = rfsm.state {
		entry=function(fsm)
			...
			state_stiffness_test(max_vel, max_acc, max_z, stiffness_custom, force_torque_limits, a_k_calc_force_start, a_k_calc_force_stop, oscillation_freq, testing_time, controller_gain)
		end,
		exit=function(fsm)
			etaslcore:stop()
			etaslcore:cleanup()
